...
function ProductCard({ product }) {
    ...
    return (
      <MDBCard className="product-card">
        <MDBCardBody className="d-flex flex-column">
          ...
          <MDBCardImage src={placeholderImage} alt="Placeholder" fluid className="card-img-top" />
  ...
  const truncatedDescription = product.description.split(" ").slice(0, 6).join(" ") + "...";
  ...
  return (
    <MDBCard className="product-card">
          ...
          <div className="mask">
            <div className="d-flex justify-content-start align-items-end h-100">
              <h5>
                ...
                {product.isEco && <span className="badge bg-success ms-2">Eco</span>}
                {product.discount > 0 && <span className="badge bg-danger ms-2">-{product.discount}%</span>}
              </h5>
            </div>
          </div>
        </a>
      ...
      <MDBCardBody className="d-flex flex-column">
        ...
        <p>{truncatedDescription}</p> {/* Display the truncated description here */}
        <div className="price-section mb-1">
          <span className="old-price">{oldPrice}</span>
          <span className="discount-price">{discountPrice}</span>
        </div>
        <div className="mt-1">
          <button className="btn btn-primary" onClick={() => addToCartHandler(product._id, 1)}>
